...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "opif-invoice-wildfly-jbpm-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name opif-invoice-wildfly-jbpm-service apps/opif-invoice-wildfly-jbpm-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app opif-invoice-wildfly-jbpm-service:1.0-SNAPSHOT
    oc expose svc/opif-invoice-wildfly-jbpm-service
else

	echo "Launching the application locally..."
	pattern="opif-invoice-wildfly-jbpm-service"
	files=( $pattern )
	cd ${files[0]}
